  ...
  return (
    <header className="flex h-14 items-center border-b border-whisper bg-canvas px-5">
      ...
      <div className="flex items-center justify-end">
        ...
        {onToggleMode && <ColorModeToggle mode={appMode} onToggle={onToggleMode} />}
        {onChangeTheme && (
...
// Light / dark toggle. Global app chrome only — course themes are
// independent and the toggle doesn't reach inside a themed surface.
function ColorModeToggle({ mode, onToggle }) {
  ...
  return (
    <button
      ...
      className="mr-2 flex h-7 w-7 items-center justify-center rounded-full border border-whisper bg-transparent text-ink-soft transition hover:bg-paper hover:text-ink"
    >
      {isDark ? (
        // Sun — means clicking will return to light.
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
          <circle cx="12" cy="12" r="4" />
          <path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41" />
        </svg>
      ) : (
        // Moon — means clicking will go to dark.
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
